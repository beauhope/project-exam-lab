...
function renderTimer() { $('#timer').textContent = formatTime(attempt.remainingSeconds); $('#timer').classList.toggle('status-bad', attempt.remainingSeconds <= 600); }
...
function render() {
  ...
  $('#previous').disabled = idx === 0 || lockedByBreak(idx - 1); $('#next').textContent = idx === questions.length - 1 ? 'إنهاء الاختبار' : 'التالي'; refreshNavigation(); announce(`السؤال ${idx + 1} من ${questions.length}`);
}
...
function confirmSubmit(){const unanswered=questions.filter(q=>!isAnswered(q,attempt.answers[q.qid])).length;$('#confirm-text').textContent=`لديك ${unanswered} سؤالًا غير مجاب. هل تريد تسليم الاختبار؟`;$('#submit-dialog').showModal()}
